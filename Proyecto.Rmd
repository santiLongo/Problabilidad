...
data_mood <- datos %>%
  group_by(Mood) %>%
  summarise(promedio_exercise = mean(Exercise..mins., na.rm = TRUE))
```

```{r}
ggplot(data_mood, aes(x = Mood, y = promedio_exercise, fill = Mood)) +
  geom_bar(stat = "identity") +
  labs(title = "Promedio de minutos de ejercicio por estado de ánimo",
       x = "Mood",
       y = "Promedio de Exercise (mins)") +
  theme_minimal()
```